...
public class UI_NicknamePopup : UI_Popup
{
    ...
    string GenerateEffectString()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if(sixstat[i] != 0)
            {
                temp += $"{(StatNameKor)i} ½Ç·Â ½ºÅÝ <sprite={i}>+{sixstat[i]} ";
            }
        }
        for (int i = 3; i < 6; i++)
        {
            if (sixstat[i] != 0)
            {
                temp += $"{(StatNameKor)i} ½ºÅÝ <sprite={i}>+{sixstat[i]} ";
            }
        }
        if (nickName.SubCount != 0)
        {
            temp += $"±¸µ¶ÀÚ ¼ö <sprite=6>+{nickName.SubCount} ";
        }
        if(nickName.MoneyValue != 0)
        {
            temp += $"°ñµå <sprite=7>+{nickName.MoneyValue}";
        }
